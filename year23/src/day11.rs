...
pub fn day11(input_lines: &[Vec<String>]) -> (String, String) {
    let galaxies = input_lines[0]
        ...
        .collect::<HashSet<Coord2>>();
    let (cols_with_g, rows_with_g): (HashSet<i64>, HashSet<i64>) =
        galaxies.iter().map(|g| (g.x, g.y)).unzip();
    let empty_rows = (0i64..(input_lines[0].len() as i64))
        .filter(|row_ix| !rows_with_g.contains(row_ix))
        .collect::<Vec<i64>>();
    let empty_cols = (0i64..input_lines[0][0].len() as i64)
        .filter(|col_ix| !cols_with_g.contains(col_ix))
        .collect::<Vec<i64>>();
    // println!("Empty rows: {:?}", empty_rows);
    // println!("Empty cols: {:?}", empty_cols);
    let (unexpanded_dist, expansions): (Vec<i64>, Vec<i64>) = galaxies
        .iter()
        .combinations(2)
        .map(|pair| {
