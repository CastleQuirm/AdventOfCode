...
pub fn day15(input_lines: &[Vec<String>]) -> (String, String) {
    let answer1 = input_lines[0][0].split(',').map(|step| hash(step)).sum::<u64>();
    
    let step_sections_regex = Regex::new(r"([a-z]+)([=-])(\d?)").unwrap();
    let instructions = step_sections_regex.captures_iter(&input_lines[0][0]).map(|cap| {
        let action = match &cap[2] {
            "=" => {
                Action::Insert { focal_length: cap[3].parse::<u64>().unwrap() }
            },
            "-" => {
                assert_eq!(&cap[3], "");
                Action::Remove
    ...
    for instruction in instructions {
        ...
        let existing_lens = frame.iter().find_position(|lens| lens.0 == instruction.label).map(|(position, _lens)| position);
    ...
    let answer2 = box_contents.iter().map(
        |(box_number, lenses)| (1 + box_number) * lenses.iter().enumerate().map(
            |(pos, (_label, focal_length))| 
                (1 + pos as u64) * focal_length
        ).sum::<u64>()
    ).sum::<u64>();
