...
pub fn day5(input_lines: &[String]) -> (u64, u64) {
    let taken_seats = input_lines.iter().map(|line| boarding_seat_as_uid(line)).collect::<Vec<u64>>();
    let min_seat = taken_seats.iter().min().expect("Inputs were empty?");
    let max_seat = taken_seats.iter().max().expect("Inputs were empty?");
    let potential_seats = (*min_seat..*max_seat).filter(|seat| !taken_seats.contains(seat)).collect::<Vec<u64>>();
...
fn boarding_seat_as_uid(seat: &String) -> u64 {
